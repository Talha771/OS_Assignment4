...
initialize(car *arr, char *filename) {
	 ...
	 */
		...
		pthread_cond_t enter_cond = PTHREAD_COND_INITIALIZER;
		pthread_mutex_t leave_lock = PTHREAD_MUTEX_INITIALIZER;
		pthread_cond_t leave_cond = PTHREAD_COND_INITIALIZER;
...
incoming_enter() {
	...
	while(outgoing_onstreet>0 || cars_on_street>=3){
		pthread_cond_wait(&enter_cond,&enter_lock);
	}
	// if(incoming_onstreet>3){
	// 	pthread_cond_wait(&enter_cond);
	// }
	incoming_onstreet++;
	cars_on_street++;
...
outgoing_enter() {
	...
	while(incoming_onstreet>0 || cars_on_street>=3){
		pthread_cond_wait(&leave_cond,&enter_lock);
	}
	// if(outgoing_onstreet>3){
	// 	pthread_cond_wait(&leave_cond);
	// }
	outgoing_onstreet++;
	cars_on_street++;
	pthread_mutex_unlock(&enter_lock);
...
incoming_leave() {
	...
	cars_on_street--;
	incoming_onstreet--;
	pthread_cond_signal(&leave_cond);
	pthread_mutex_unlock(&enter_lock);
}
